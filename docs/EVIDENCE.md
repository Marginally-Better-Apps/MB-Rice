...
| Ricepack import and export | Implemented for stored ZIP | Yes | Seven unit tests passed on iOS 26.5, including three sample packs | Pending | No |
| Wallpaper and icon PNG export | Implemented | Yes | Pending export UI test | Pending setup test | No |
...
The simulator launch capture is [library-iphone17pro-ios26.5.png](captures/library-iphone17pro-ios26.5.png). It is an actual app screenshot. It is not a widget placement capture. Seven unit tests passed with `xcodebuild test` after temporarily setting deployment to 26.5 for the simulator, then restoring 27.0 in the project.
