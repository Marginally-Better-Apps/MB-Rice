...
| App Group state round trip | Implemented with local app fallback and migration | Yes | Ad hoc signed simulator install created App Group state with existing saved themes; storage migration and widget selection tests passed | Pending Autoloader profile check | No |
